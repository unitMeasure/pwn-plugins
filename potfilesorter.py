#https://github.com/itsdarklikehell/pwnagotchi-plugins/blob/master/potfilesorter.py
import sys
import os
import subprocess
import json
...
class potfilesorter(plugins.Plugin):
    ...
    # potfile_source = '/home/pi/wpa-sec.cracked.potfile'
    ...
    def on_loaded(self):
        ...
        if not self.options["api_url"]:
            logging.error(
                "[wpasec] API-URL isn't set. Can't upload, no endpoint configured."
            )
            return
    ...
    def backup_configs(
        ...
        wificonfigstore_backup,
        wificonfigstoresoftap_tmp,
        wificonfigstoresoftap_source,
        ...
        with open(wpa_tmp, "r") as checklines:
            for line in checklines:
                if search_str in line:
                    logging.info(
                        search_str + " is already in the file: " + checklines.name
                    )
                    return True
    ...
    def readpotfiledata(
        ...
        with open(
            os.path.join(handshake_dir, "wpa-sec.cracked.potfile"), "r"
        ) as checkpotfile:
